...
# ./Horkan_backup.sh TMP/

#Part 1: Check if the directory BACKUP_FILES exists
...
[ ! -d "$HOME/BACKUP_FILES" ] && sudo mkdir $HOME/BACKUP_FILES && echo "Created BACKUP_FILES in $HOME directory"
 

#Part 2: Write a for loop that copies all the files of a directory into BACKUP_FILES
#  The directory needs to be specified as the first option after the script (see above).

#  Hint: see the script 'convert_h0tml_to_php.sh' at:
#     https://ryanstutorials.net/bash-scripting-tutorial/bash-loops.php

#echo "$fn copied to $HOME/BACKUP_FILES"
for filepath in $1*
do 
	cp $filepath $HOME/BACKUP_FILES/$( basename $filepath )
done 
